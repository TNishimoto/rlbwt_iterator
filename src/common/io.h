...
class IO
{
  public:
	...
	//static bool write(std::string &filename, std::vector<int32_t> &text);
	...
	static uint64_t online_write(std::ofstream &os, ITERATOR &&beginIt, ITERATOR &&endIt, uint64_t bufferSize = 1000, bool isPrint = false, std::string msg = "items")
	{
		uint64_t count = 0;
		...
		while (beginIt != endIt)
		{
			...
			if (isPrint)
			{
				...
				//std::cout << print(*beginIt) << std::endl;
			}
