...
class Printer
{
  public:
	...
	template <class X>
	static std::string toString(const std::vector<X> &items, uint64_t padding)
	{
		std::string s = "";
		toIntegerString<X>(items, s, padding);
		return s;
	}
	static std::string toString(const std::string &str, uint64_t padding)
	{
		std::string s = "";
	...
	template <class X>
	static void write(std::string filename, const std::vector<X> &items)
	{
	...
	static bool write(std::ofstream &os, const std::string &text)
	{
		os.write((const char *)(&text[0]), sizeof(char) * text.size());
		return true;
	}
	static bool write(std::string filename, const std::string &text)
	{
		std::ofstream out(filename, std::ios::out | std::ios::binary);
		if (!out)
			return 1;
	...
	static bool debugWrite(const std::string &text)
	{
	...
	template <class X>
	static void debugWrite(const std::vector<X> &items)
	{
		std::ofstream out("debug.txt", std::ios::out | std::ios::binary | std::ios::app);
		if (!out)
		{
