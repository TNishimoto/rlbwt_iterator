...
class InitialDataStructureForSamplingLCP{
    public:
    std::vector<uint64_t> indexes;
    std::vector<uint64_t> intervals;
    std::vector<uint64_t> prev_mapper;
    std::vector<uint64_t> sampling_lcp;
    
    template <typename CHAR = char>
    void construct_initial_data(const RLBWT<CHAR> &_rlbwt)
    {
        RLEFArrayGenerator<CHAR, uint64_t, RLBWT<CHAR>> generator(_rlbwt);
        
        this->prev_mapper.resize(_rlbwt.rle_size());
        ...
        {
            std::cout << "!" << it.rle_findex() << std::endl;
            ...
            {
                this->sampling_lcp[rle_lindex] = 0;
                uint64_t findex = it.findex();
                this->indexes.push_back(findex);
                this->prev_mapper[rle_lindex] = std::numeric_limits<uint64_t>::max();
            }
            else
            {
                this->intervals.push_back(rle_lindex);
                this->prev_mapper[rle_lindex] = prev_rle_lindex;
            }
            prev_c = c;
            prev_rle_lindex = rle_lindex;
        }
        std::cout << "output_intervals" << std::endl;
        Printer::print(this->intervals);
        std::cout << "output_sampling_lcp" << std::endl;
        Printer::print(this->sampling_lcp);
