...
class SelectOnRLBWT{
    ...
    std::vector<uint64_t> next_c_index_vector;
    ...
    void build(RLBWT_STR &_rlbwt){
        ...
        this->next_c_index_vector.resize(_rlbwt.rle_size(), UINT64_MAX);
...
class RLBWTRightIntervals{
    public:
    std::vector<uint64_t> *next_c_index_vector;
    RLBWTRightIntervals(std::vector<uint64_t> *_next_c_index_vector) : next_c_index_vector(_next_c_index_vector) {
...
public:
    ...
    static std::vector<uint64_t> construct_zero_lcp_findexes(SelectOnRLBWT &selecter, std::vector<uint64_t> &_findexes_lorder){
    ...
    static std::vector<bool> construct_interval_flag_vec(SelectOnRLBWT &selecter){
        ...
        for(uint64_t i=0;i<selecter.next_c_index_vector.size();i++){
            interval_flag_vec[i] = selecter.next_c_index_vector[i] != UINT64_MAX;
        }

        return interval_flag_vec;
        
    }
    static std::vector<uint64_t> loop(const RLBWT_STR &_rlbwt)
    {
        uint64_t run_size = _rlbwt.rle_size();
        SelectOnRLBWT selecter;
        ...
        RLBWTRightIntervals right_intervals(&selecter.next_c_index_vector);
